...
def generate_launch_description():
    # Arguments
    pkg_arg = DeclareLaunchArgument("pkg", default_value="tiago_pro_lfc_bringup")
    lfc_yaml_arg = DeclareLaunchArgument(
        "lfc_yaml", default_value="config/fixed/linear_feedback_controller_params.yaml"
    )
    jse_yaml_arg = DeclareLaunchArgument(
        "jse_yaml", default_value="config/fixed/joint_state_estimator_params.yaml"
    )
    ...
    # Deactivate the old arms controller if present
    deactivate_arm = ExecuteProcess(
        cmd=["ros2", "control", "switch_controllers", "--deactivate", "arm_right_controller"],
        output="screen",
    )
    # deactivate_arm = ExecuteProcess(
    #     cmd=["ros2", "control", "switch_controllers", "--deactivate", "arm_left_controller"],
    #     output="screen",
    # )
